...
const Faq: React.FC = () => {
  ...
  const filteredQuestions = faqData.filter(faq => faq.section === selectedCard);
    ...
    <section className="my-5">
      ...
      <div className="flex flex-col w-full px-8 md:px-8">
        {filteredQuestions.map((faq, index) => (
          <div key={index}>
            <button onClick={() => handleQuestionClick(index)} className="rounded-lg p-2 hover:text-primary hover:bg-secondary focus:outline-none delay-50 transition-all duration-300 ease-in-out">
              {faq.question}
            </button>
            <div className={`px-8 md:px-8 mt-2 transition-opacity duration-300 opacity-${selectedQuestionIndex === index ? '100' : '0'}`}>
