...
function FaqCard({ image, title, description }: CardProps) {
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-0 flex gap-2 flex-col justify-center items-center px-4 md:px-8 py-5 opacity-${
          hoveredMain ? '0' : '100'
      ...
      >
        <h3 className="text-primary text-sm text-center">
          {hoveredMain ? description : ''}
        </h3>
      </div>
    </div>
  )
